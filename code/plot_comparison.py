...
# =======================
# Filter area and keep relevant columns
# =======================
...
df = (
    df_np
    .merge(df_norm, on="date", how="inner")
    .merge(df_logn, on="date", how="inner")
    .merge(df_bkm, on="date", how="inner")
    .sort_values("date")
)

print(f"Aligned dates used: {len(df)}")
...
plt.plot(df["date"], df["mean_norm"], label="Parametric Normal", linewidth=1, alpha=0.5)
...
plt.plot(df["date"], df["var_norm"], label="Parametric Normal", linewidth=1, alpha=0.5)

plt.yscale("log")
plt.title(f"Implied Variance Over Time ({AREA}) — log scale")
plt.legend()
plt.tight_layout()
...
# =======================
# NUMERICAL COMPARISON (MANDATORY)
# =======================
...
print("\nVariance level NP:", df["var_np"].mean())
...
print("\n=== Largest disagreements (NP vs BKM) ===")
print("\nTop mean gaps:")
